...
#Autenticação
ee.Authenticate()
ee.Initialize(project='ee-usuariogee')

# Criar um mapa interativo
Map = geemap.Map()

# Carregar o shapefile (Área de Estudo)
shapefile = ee.FeatureCollection('inserirshape') #Carrega um shapefile da pasta de assets no Earth Engine
area = shapefile.geometry() #Extrai apenas a geometria da área

# Carregar coleção Landsat 8
image_collection = (
    ee.ImageCollection("LANDSAT/LC08/C02/T1_L2") #Inserir satélite
    .filterDate('datainicial', 'datafinal') #Inserir data
    .filterBounds(area)
    .filter(ee.Filter.inList('WRS_PATH', [225])) #Retirar cenas que peguem a "beirada" da TI: inserir cenas
    .filter(ee.Filter.inList('WRS_ROW', [64])) #Inserir cenas
